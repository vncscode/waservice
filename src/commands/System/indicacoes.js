...
module.exports = createCommand({
    name: "indicacoes",
    params: "<-start> <-stop>",
    aliases: ["convidarmember", "indicações"],
    isAdmin: true,
    menu: "system",
    desc: "Habilita o sistema de Indicações para Grupo.",
    async run(int) {
        try {
            ...
            if (!query) {
                await sock.sendMessage(from, {
                    text: `${Config.get("name")}: Para utilizar esse comando, insira -start para Habilitar as Indicações ou -stop para desabilitalo.`
                }, messageOptions);
                return;
            }
            ...
            if (query.includes('-start')) {
                ...
                await sock.sendMessage(from, {
                    text: `${Config.get("name")}: Indicações ativado com sucesso para este grupo!`
                }, messageOptions);
                ...
                await saveConfig(fileconf);
                await sock.sendMessage(from, {
                    text: `${Config.get("name")}: Indicações desativado com sucesso para este grupo!`
                }, messageOptions);
            } else {
                await sock.sendMessage(from, {
            ...
            console.error('Erro no comando Indicações:', e);
            const duration = await int.getEphemeralDuration(sock, from);
            await sock.sendMessage(
                from,
