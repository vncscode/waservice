...
module.exports = createCommand({
    ...
    isAdmin: true,
    menu: "grupos",
    desc: "Captcha protege o Grupo contra usuarios Fantasmas.",
    async run(int) {
        try {
            ...
            if (!query) {
                await sock.sendMessage(from, {
                    text: `${Config.get("name")}: Para utilizar esse comando, insira -start para Habilitar o Captcha ou -stop para desabilitalo.`
                }, messageOptions);
                return;
            }
            ...
            if (query.includes('-start')) {
                ...
                await sock.sendMessage(from, {
                    text: `${Config.get("name")}: Captcha ativado com sucesso para este grupo!`
                }, messageOptions);
                ...
                await saveConfig(fileconf);
                await sock.sendMessage(from, {
                    text: `${Config.get("name")}: Captcha desativado com sucesso para este grupo!`
                }, messageOptions);
            } else {
                await sock.sendMessage(from, {
            ...
            console.error('Erro no comando captcha:', e);
            const duration = await int.getEphemeralDuration(sock, from);
            await sock.sendMessage(
                from,
